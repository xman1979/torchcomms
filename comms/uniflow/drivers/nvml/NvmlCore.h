...
// #define UNIFLOW_NVML_DIRECT 1
#ifndef UNIFLOW_NVML_DIRECT
#define UNIFLOW_NVML_DIRECT 0
#endif
...
typedef struct nvmlPciInfo_st {
  char
      busId[NVML_DEVICE_PCI_BUS_ID_BUFFER_SIZE]; //!< The tuple
                                                 //!< domain:bus:device.function
                                                 //!< PCI identifier (&amp;
                                                 //!< NULL terminator)
  unsigned int
      domain; //!< The PCI domain on which the device's bus resides, 0 to 0xffff
...
/* NVLink Speed */
#define NVML_FI_DEV_NVLINK_SPEED_MBPS_COMMON \
 ...
 */
#define NVML_FI_DEV_NVSWITCH_CONNECTED_LINK_COUNT \
  147 //!< Number of NVLinks connected to NVSwitch

#define NVML_FI_DEV_NVLINK_GET_SPEED 164
#define NVML_FI_DEV_NVLINK_GET_STATE 165
#define NVML_FI_DEV_NVLINK_GET_VERSION 166

#define NVML_FI_DEV_C2C_LINK_COUNT \
  170 //!< Number of C2C Links present on the device
#define NVML_FI_DEV_C2C_LINK_GET_STATUS \
  171 //!< C2C Link Status 0=INACTIVE 1=ACTIVE
#define NVML_FI_DEV_C2C_LINK_GET_MAX_BW \
  172 //!< C2C Link Speed in MBps for active links
...
typedef struct nvmlFieldValue_st {
  ...
  unsigned int
      scopeId; //!< Scope ID can represent data used by NVML depending on
               //!< fieldId's context. For example, for NVLink throughput
               //!< counter data, scopeId can represent linkId.
  long long
...
typedef struct {
  unsigned char
      clusterUuid[NVML_GPU_FABRIC_UUID_LEN]; //!< Uuid of the cluster to which
                                             //!< this GPU belongs
  nvmlReturn_t status; //!< Error status, if any. Must be checked only if state
                       //!< returns "complete".
 ...
 */
typedef struct {
  unsigned int version; //!< Structure version identifier (set to \ref
...
typedef struct {
  ...
  unsigned char
      ibGuid[16]; //!< Infiniband GUID reported by platform (for Blackwell,
                  //!< ibGuid is 8 bytes so indices 8-15 are zero)
  unsigned char
      chassisSerialNumber[16]; //!< Serial number of the chassis containing this
                               //!< GPU (for Blackwell it is 13 bytes so indices
                               //!< 13-15 are zero)
  unsigned char slotNumber; //!< The slot number in the chassis containing this
                            //!< GPU (includes switches)
  unsigned char
      trayIndex; //!< The tray index within the compute slots in the chassis
                 //!< containing this GPU (does not include switches)
  unsigned char
      hostId; //!< Index of the node within the slot containing this GPU
  unsigned char peerType; //!< Platform indicated NVLink-peer type (e.g. switch
                          //!< present or not)
