...
namespace {
  template<typename T, typename RedOp, typename Proto, int COLL_UNROLL, bool isNetOffload = false>
...
#define rcclAllGatherRunRingSimpleProtoImpl(tid, nthreads, work) \
  if(work->rcclUseOneSlice){ \
    runRing<T, RedOp, ProtoSimple<ALLGATHER_CHUNKSTEPS/ALLGATHER_SLICESTEPS_SINGLE_NODE, ALLGATHER_SLICESTEPS_SINGLE_NODE, COLL_UNROLL>, COLL_UNROLL, false>(tid, nthreads, work); \
  } else{ \
    runRing<T, RedOp, ProtoSimple<ALLGATHER_CHUNKSTEPS/ALLGATHER_SLICESTEPS, ALLGATHER_SLICESTEPS, COLL_UNROLL>, COLL_UNROLL, false>(tid, nthreads, work); \
  }
#else
#define rcclAllGatherRunRingSimpleProtoImpl(tid, nthreads, work) \
  runRing<T, RedOp, ProtoSimple<ALLGATHER_CHUNKSTEPS/ALLGATHER_SLICESTEPS, ALLGATHER_SLICESTEPS, COLL_UNROLL>, COLL_UNROLL, false>(tid, nthreads, work);
...
struct RunWorkColl<ncclFuncAllGather, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_SIMPLE, COLL_UNROLL> {
...
#endif
    if (isNetOffload)
      runRing<T, RedOp, ProtoSimple<1, 1, COLL_UNROLL>, COLL_UNROLL, true>(tid, nthreads, work);
    else{
...
template<typename T, typename RedOp, int COLL_UNROLL>
struct RunWorkColl<ncclFuncAllGather, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_LL, COLL_UNROLL> {
  __device__ __forceinline__ void run(int tid, int nthreads, struct ncclDevWorkColl* work) {
    runRing<T, RedOp, ProtoLL, COLL_UNROLL>(tid, nthreads, work);
  }
};

template<typename T, typename RedOp, int COLL_UNROLL>
struct RunWorkColl<ncclFuncAllGather, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_LL128, COLL_UNROLL> {
  __device__ __forceinline__ void run(int tid, int nthreads, struct ncclDevWorkColl* work) {
    runRing<T, RedOp, ProtoLL128, COLL_UNROLL>(tid, nthreads, work);
  }
};

template<typename T, typename RedOp, int COLL_UNROLL>
struct RunWorkColl<ncclFuncAllGather, T, RedOp, NCCL_ALGO_PAT, NCCL_PROTO_SIMPLE, COLL_UNROLL> {
  __device__ __forceinline__ void run(int tid, int nthreads, struct ncclDevWorkColl* work) {
...
template<typename T, typename RedOp, int COLL_UNROLL>
struct RunWorkColl<ncclFuncAllGather, T, RedOp, NCCL_ALGO_NVLS, NCCL_PROTO_SIMPLE, COLL_UNROLL> {
  __device__ __forceinline__ void run(int tid, int/*nthreads*/, struct ncclDevWorkColl* work) {
...
template<typename T, typename RedOp, int COLL_UNROLL>
struct RunWorkColl<ncclFuncAllGather, T, RedOp, NCCL_ALGO_COLLNET_DIRECT, NCCL_PROTO_SIMPLE, COLL_UNROLL> {
