...
NCCL_IR_EXTERN_C NCCL_DEVICE_INLINE void*
ncclGetPeerPointerTeam(ncclWindow_t w, size_t offset, ncclTeam tm, int peer) {
  return ncclGetPeerPointer(w, offset, tm, peer);
}

/* coop */
NCCL_IR_EXTERN_C NCCL_DEVICE_INLINE void ncclCoopAnyInitThread(
    ncclCoopAny* coop) {
  ::new (coop) ncclCoopAny(ncclCoopThread());
}
NCCL_IR_EXTERN_C NCCL_DEVICE_INLINE void ncclCoopAnyInitWarp(
    ncclCoopAny* coop) {
  ::new (coop) ncclCoopAny(ncclCoopWarp());
}
NCCL_IR_EXTERN_C NCCL_DEVICE_INLINE void ncclCoopAnyInitLanes(
    ncclCoopAny* coop,
    uint32_t lane_mask) {
...
NCCL_IR_EXTERN_C NCCL_DEVICE_INLINE int ncclCoopThreadRank(
    const ncclCoopAny* coop) {
  return coop->thread_rank();
}
NCCL_IR_EXTERN_C NCCL_DEVICE_INLINE int ncclCoopSize(const ncclCoopAny* coop) {
  return coop->size();
}
NCCL_IR_EXTERN_C NCCL_DEVICE_INLINE int ncclCoopNumThreads(
    const ncclCoopAny* coop) {
...
NCCL_IR_EXTERN_C NCCL_DEVICE_INLINE void ncclLsaBarrierSessionArrive(
    ncclLsaBarrierSession_C* session,
    ncclCoopAny coop,
    cuda::memory_order order) {
  session->bar.arrive(coop, order);
}
NCCL_IR_EXTERN_C NCCL_DEVICE_INLINE void ncclLsaBarrierSessionWait(
    ncclLsaBarrierSession_C* session,
    ncclCoopAny coop,
    cuda::memory_order order) {
  session->bar.wait(coop, order);
}
NCCL_IR_EXTERN_C NCCL_DEVICE_INLINE void ncclLsaBarrierSessionSync(
    ncclLsaBarrierSession_C* session,
    ncclCoopAny coop,
    cuda::memory_order order) {
...
NCCL_IR_EXTERN_C NCCL_DEVICE_INLINE void ncclBarrierSessionInit(
  ...
  ::new (&(session->bar)) ncclBarrierSession<ncclCoopAny>(
      ...
      innerTeam,
      outerTeam,
      reinterpret_cast<ncclGin const&>(net),
      innerBarHandle,
      outerBarHandle,
      index,
      multimem,
