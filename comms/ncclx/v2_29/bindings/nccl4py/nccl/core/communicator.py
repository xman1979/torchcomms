...
from nccl.core.cuda import get_cuda_device, get_stream_ptr
...
class NCCLConfig:
    ...
    def __init__(
        ...
        *,
        ...
        self._cfg.magic = (
            0xCAFEBEEF  # NCCL protocol magic number for ncclConfig_t validation
        )
...
class Communicator:
    ...
    def __init__(self, ptr: int) -> None:
        ...
        self._device = (
            Device(int(_nccl_bindings.comm_cu_device(self._comm))) if ptr != 0 else None
        )
        self._rank = (
            int(_nccl_bindings.comm_user_rank(self._comm)) if ptr != 0 else None
        )
    ...
    # --- Communicator APIs ---
    def split(
        self, color: int, key: int, config: NCCLConfig | None = None
    ) -> Communicator:
        """
    ...
    def wait_signal(
        self,
        signal_descs: Sequence[WaitSignalDesc],
        *,
        stream: NcclStreamSpec | None = None,
    ) -> None:
        """
        ...
        for idx, desc in enumerate(signal_descs):
            if not isinstance(desc, WaitSignalDesc):
                raise NcclInvalid(
                    f"Descriptor at index {idx} is not a valid WaitSignalDesc"
                )
    ...
    def signal(
        ...
        *,
        ...
        _nccl_bindings.signal(
            peer, sig_idx, ctx, flags, self._comm, get_stream_ptr(stream)
        )
