...
// NCCLX: float specialization for quantized reduce scatter via PAT algorithm.
// When quantizeRandomSeedPtr is set, uses PrimitivesQuantized for mixed-precision
// transport (FP32 I/O, BF16 transport with stochastic rounding).
// When nullptr, falls through to regular Primitives (same as generic template).
  ...
  __device__ __forceinline__ void run(int tid, int nthreads, struct ncclDevWorkColl* work) {
    ...
    if (tid == nworkers) { // Algo computation thread
      ...
      if (work->quantizeRandomSeedPtr != nullptr) {
        ...
        PrimitivesQuantized<float, nv_bfloat16, RedOp, 1, COLL_UNROLL> primsQuantized
