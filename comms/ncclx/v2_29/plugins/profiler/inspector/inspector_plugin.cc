...
inspectorResult_t inspectorPluginCollInfoRefSafe(
    struct inspectorCollInfo* collInfo) {
...
inspectorResult_t inspectorPluginCollInfoDeRefSafe(
    struct inspectorCollInfo* collInfo) {
  inspectorLockWr(&collInfo->guard);
    ...
    if (parentType == ncclProfileColl) {
      ...
      if (collInfo && collInfo->type == ncclProfileColl) {
        ...
        if (kernelChInfoPtr->stopGpuClk == 0) {
          inspectorPluginCollInfoRef(
              collInfo); // Pairs with Record Kernel Stop event
        }
...
__hidden ncclResult_t inspectorPluginStopEvent(void* eHandle) {
  ...
  if (type == ncclProfileColl) {
    ...
    inspectorLockWr(&collInfo->guard);
    inspectorRecordEventTrace(
        collInfo->collEvtTrk.evntTrace, NCCL_INSP_EVT_TRK_COLL_STOP, collInfo);
    res = inspectorPluginCollInfoDeRef(collInfo);
    if (res == inspectorReturn) {
      // WARN("NCCL Inspector unnatural return:
    ...
    if (collInfo && collInfo->type == ncclProfileColl) {
      ...
      if (res == inspectorReturn) {
        INFO_INSPECTOR(
            "NCCL Inspector unnatural return: inspectorPluginStopEvent:ncclProfileKernelCh");
        return ncclSuccess;
      }
      if ((collInfo->nKernelChCompleted == collInfo->nKernelChStarted) &&
        ...
        if (commInfo != nullptr) {
          ...
          inspectorComputeCollBw(commInfo, &completedColl, completedColl.func);
          memcpy(
  ...
  if (type == ncclProfileKernelCh && eState == ncclProfilerKernelChStop) {
    ...
    if (collInfo && collInfo->type == ncclProfileColl) {
      ...
      if (kernelChInfo->startGpuClk != 0) {
        ...
        if (res == inspectorReturn) {
          INFO_INSPECTOR(
              "NCCL Inspector unnatural return: inspectorPluginRecordEventState");
          return ncclSuccess;
        }
      }
      inspectorUnlockRWLock(&collInfo->guard);
