 ...
 *   None.
 */
static void inspectorFormatHumanReadableSize(
    size_t bytes,
    char* output,
    size_t outputSize) {
 ...
 * Description:
 *
 *   Caches the static parts of Prometheus labels (hostname, job info, comm
 * info) for a communicator. This should be called once when the communicator is
 * created.
 *
 * Thread Safety:
 ...
 */
inspectorResult_t inspectorPromCacheStaticLabels(
  ...
  int ret = snprintf(
      ...
      sizeof(commInfo->cachedStaticLabels),
 ...
 * Description:
 *
 *   Formats labels for Prometheus metrics from communicator and collective
 * info. Uses cached static labels and only adds dynamic parts (collective,
 * timestamp, etc).
 *
 * Thread Safety:
 *
 ...
 *   struct inspectorCommInfo* commInfo - communicator info (with cached static
 * labels). struct inspectorCompletedCollInfo* collInfo - completed collective
 * info.
 *
 * Output:
 ...
 *   inspectorResult_t - success or error code.
 */
static inspectorResult_t inspectorPromGetLabels(
    char* labels,
    size_t labelSize,
    struct inspectorCommInfo* commInfo,
  ...
  char msgSizeStr[32];
  inspectorFormatHumanReadableSize(
      collInfo->msgSizeBytes, msgSizeStr, sizeof(msgSizeStr));
 ...
 *   inspectorResult_t - success or error code.
 */
static inspectorResult_t inspectorPromWriteCollInfo(
    FILE* file,
    struct inspectorCommInfo* commInfo,
    struct inspectorCompletedCollInfo* collInfo) {
  if (!file) {
  ...
  INS_CHK(inspectorPromGetLabels(labels, sizeof(labels), commInfo, collInfo));
...
static inspectorResult_t inspectorPromCommInfoDump(
  ...
  *needs_writing = false;
  ...
  if (*needs_writing) {
    TRACE_INSPECTOR(
        "NCCL Inspector: Writing metrics for comm %s directly",
        commInfo->commHashStr);
    INS_CHK(inspectorPromWriteCollInfo(file, commInfo, &collInfo));
  }
...
inspectorResult_t inspectorPromCommInfoListDump(
  ...
  if (commList->ncomms > 0) {
    ...
    for (struct inspectorCommInfo* itr = commList->comms; itr != nullptr;
      ...
      char filename[1024];
      INS_CHK_GOTO(
          inspectorPromGetFilename(
              output_root, itr->deviceUuidStr, filename, sizeof(filename)),
          res,
          exit);

      FILE* file = dumpThread ? dumpThread->getOrCreateFileHandle(
                                    itr->deviceUuidStr, filename, currentTime)
                              : NULL;
      if (!file) {
        INFO_INSPECTOR(
      ...
      INS_CHK_GOTO(
          inspectorPromCommInfoDump(itr, file, &needs_writing), res, exit);
