...
static inline inspectorResult_t inspectorCompletedCollVerbose(
  ...
  JSON_CHK(jsonStartObject(jfo));
  {
    ...
    JSON_CHK(jsonUint64(
        jfo, collInfo->collEvtTrk.evntTrace[NCCL_INSP_EVT_TRK_COLL_START].sn));
    JSON_CHK(jsonKey(jfo, "coll_stop_sn"));
    JSON_CHK(jsonUint64(
        jfo, collInfo->collEvtTrk.evntTrace[NCCL_INSP_EVT_TRK_COLL_STOP].sn));
    ...
    for (uint32_t ch = 0; ch < collInfo->collEvtTrk.nChannels; ch++) {
      ...
      JSON_CHK(jsonUint64(
          jfo,
          collInfo->collEvtTrk.kernelCh[ch]
      ...
      JSON_CHK(jsonUint64(
          jfo,
          collInfo->collEvtTrk.kernelCh[ch]
              .evntTrace[NCCL_INSP_EVT_TRK_KERNEL_STOP]
              .sn));
  ...
  JSON_CHK(jsonStartObject(jfo));
  {
    ...
    JSON_CHK(jsonUint64(
        jfo, collInfo->collEvtTrk.evntTrace[NCCL_INSP_EVT_TRK_COLL_START].ts));
    JSON_CHK(jsonKey(jfo, "coll_stop_ts"));
    JSON_CHK(jsonUint64(
        jfo, collInfo->collEvtTrk.evntTrace[NCCL_INSP_EVT_TRK_COLL_STOP].ts));
...
static inline inspectorResult_t inspectorCompletedColl(
  ...
  JSON_CHK(jsonStartObject(jfo));
  {
    ...
    JSON_CHK(jsonStr(jfo, ncclFuncToString(collInfo->func)));

    JSON_CHK(jsonKey(jfo, "coll_sn"));
    JSON_CHK(jsonUint64(jfo, collInfo->sn));

    JSON_CHK(jsonKey(jfo, "coll_msg_size_bytes"));
    JSON_CHK(jsonUint64(jfo, collInfo->msgSizeBytes));

    JSON_CHK(jsonKey(jfo, "coll_exec_time_us"));
    JSON_CHK(jsonUint64(jfo, collInfo->execTimeUsecs));

    JSON_CHK(jsonKey(jfo, "coll_timing_source"));
    JSON_CHK(
        jsonStr(jfo, inspectorTimingSourceToString(collInfo->timingSource)));

    JSON_CHK(jsonKey(jfo, "coll_algobw_gbs"));
    JSON_CHK(jsonDouble(jfo, collInfo->algoBwGbs));

    JSON_CHK(jsonKey(jfo, "coll_busbw_gbs"));
    JSON_CHK(jsonDouble(jfo, collInfo->busBwGbs));
 ...
 *   struct inspectorCommInfoList* commList - list of communicators (must not be
 * NULL).
 *
 * Output:
 ...
 *
 */
static inspectorResult_t inspectorCommInfoListDump(
    jsonFileOutput* jfo,
    struct inspectorCommInfoList* commList) {
  bool flush = false;
...
inspectorResult_t inspectorDumpThread::inspectorStateDumpProm(
    const char* output_root) {
  // Write communicators directly to files with per-device flushing handled
  // inside
...
static inspectorResult_t inspectorStartDumpThread(uint64_t intervalUsecs) {
  if (intervalUsecs == 0) {
    INFO_INSPECTOR(
        "NCCL Inspector: dump thread enabled but "
  ...
  struct {
    ...
    const char* description;
      ...
      {"NCCL_INSPECTOR_DUMP_THREAD_INTERVAL_MICROSECONDS",
       ...
       "0",
       "Dump thread interval in microseconds"},
...
void inspectorComputeCollBw(
  ...
  completedColl->algoBwGbs =
      timeInSec != 0 ? (trafficSize / 1.0E9 / timeInSec) : 0;
