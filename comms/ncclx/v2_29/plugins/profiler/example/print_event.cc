...
// FIXME: chrome tracing asynchronous events (following used) allow event
// nesting for events that have same id and category It appears that nesting
// more than three events causes issues. Therefore, every event is given an
// increasing id and a category that matches the type of event (GROUP API, COLL
// API, P2P API, GROUP, COLL, P2P, PROXY, NET)
static __thread int groupApiId;
__hidden void printGroupApiEventHeader(FILE* fh, struct groupApi* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"GROUP_API\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"groupApiId\": %d, \"groupDepth\": %d}},\n",
      "Group API",
      groupApiId,
      getpid(),
      1,
...
__hidden void printGroupApiEventTrailer(FILE* fh, struct groupApi* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"GROUP_API\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
      "Group API",
      groupApiId++,
      getpid(),
      1,
...
__hidden void printP2pApiEventHeader(FILE* fh, struct p2pApi* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"P2P_API\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"count\": %lu, \"datatype\": \"%s\", \"GraphCaptured\": %d, \"Stream\": \"%p\"}},\n",
      event->func,
      p2pApiId,
      getpid(),
      1,
...
__hidden void printP2pApiEventTrailer(FILE* fh, struct p2pApi* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"P2P_API\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
      event->func,
      p2pApiId++,
      getpid(),
      1,
...
__hidden void printCollApiEventHeader(FILE* fh, struct collApi* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"COLL_API\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"count\": %lu, \"datatype\": \"%s\", \"root\": %d, \"GraphCaptured\": %d, \"Stream\": \"%p\"}},\n",
      event->func,
      collApiId,
      getpid(),
      1,
...
__hidden void printCollApiEventTrailer(FILE* fh, struct collApi* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"COLL_API\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
      event->func,
      collApiId++,
      getpid(),
      1,
...
__hidden void printKernelLaunchEventHeader(
    FILE* fh,
    struct kernelLaunch* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"KERNEL_LAUNCH\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"groupId\": %d, \"Stream\": \"%p\"}},\n",
      "KernelLaunch",
      kernelLaunchId,
      getpid(),
      1,
      event->startTs,
      event->kernelLaunchId,
      event->stream);
}

__hidden void printKernelLaunchEventTrailer(
    FILE* fh,
    struct kernelLaunch* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"KERNEL_LAUNCH\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
      "KernelLaunch",
      kernelLaunchId++,
      getpid(),
      1,
...
__hidden void printGroupEventHeader(FILE* fh, struct group* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"GROUP\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"groupId\": %d}},\n",
      "Group",
      groupId,
      getpid(),
      1,
...
__hidden void printGroupEventTrailer(FILE* fh, struct group* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"GROUP\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
      "Group",
      groupId++,
      getpid(),
      1,
...
__hidden void printCollEventHeader(FILE* fh, struct collective* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"COLL\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"SeqNum\": %lu, \"CommHash\": %lu, \"Rank\": %d, \"Count\": %lu, \"Datatype\": \"%s\", \"Algorithm\": \"%s\", \"Protocol\": \"%s\", \"nChannels\": %d}},\n",
      event->base.func,
      collId,
      getpid(),
      1,
      event->base.startTs,
      event->seqNumber,
      ((struct collApi*)event->base.parent)->ctx->commHash,
      event->base.rank,
      event->count,
      event->datatype,
      event->algo,
...
__hidden void printCollEventTrailer(FILE* fh, struct collective* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"COLL\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
      event->base.func,
      collId++,
      getpid(),
      1,
...
__hidden void printP2pEventHeader(FILE* fh, struct p2p* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"P2P\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"CommHash\": %lu, \"Rank\": %d, \"Peer\": %d, \"Count\": %lu, \"Datatype\": \"%s\", \"nChannels\": %d}},\n",
      event->base.func,
      p2pId,
      getpid(),
      1,
      event->base.startTs,
      ((struct p2pApi*)event->base.parent)->ctx->commHash,
      event->base.rank,
      event->peer,
      event->count,
      event->datatype,
...
__hidden void printP2pEventTrailer(FILE* fh, struct p2p* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"P2P\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
      event->base.func,
      p2pId++,
      getpid(),
      1,
...
__hidden void printProxyOpEventHeader(FILE* fh, struct proxyOp* event) {
  if (event->isSend) {
    fprintf(
        fh,
        "{\"name\": \"%s\", \"cat\": \"PROXY\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"Channel\": %d, \"Peer\": %d, \"Steps\": %d, \"ChunkSize\": %d, \"transSize\": %lu}},\n",
        "ScheduleSend",
        proxyOpId,
        getpid(),
        1,
    ...
    fprintf(
        fh,
        "{\"name\": \"%s\", \"cat\": \"PROXY\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
        "ScheduleSend",
        proxyOpId,
        getpid(),
        1,
...
__hidden void printProxyOpEventTrailer(FILE* fh, struct proxyOp* event) {
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"PROXY\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
      event->isSend ? "ProgressSend" : "ProgressRecv",
      proxyOpId++,
      getpid(),
      1,
      event->stopTs);
...
__hidden void printProxyStepEventHeader(FILE* fh, struct proxyStep* event) {
  if (event->isSend) {
    fprintf(
        fh,
        "{\"name\": \"%s\", \"cat\": \"NET\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"Step\": %d}},\n",
        "SendGpuWait",
        proxyStepId,
        getpid(),
        1,
        event->timestamp[PROXY_STEP_SEND_GPU_WAIT],
        event->step);
    fprintf(
        fh,
        "{\"name\": \"%s\", \"cat\": \"NET\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
        "SendGpuWait",
        proxyStepId,
        getpid(),
        1,
        event->timestamp[PROXY_STEP_SEND_PEER_WAIT]);
    fprintf(
        fh,
        "{\"name\": \"%s\", \"cat\": \"NET\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"Step\": %d}},\n",
        "SendPeerWait",
    ...
    fprintf(
        ...
        getpid(),
        1,
        event->timestamp[PROXY_STEP_SEND_WAIT]);
    fprintf(
        fh,
        "{\"name\": \"%s\", \"cat\": \"NET\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"Step\": %d}},\n",
        "SendWait",
    ...
    fprintf(
        ...
        getpid(),
        1,
        event->timestamp[PROXY_STEP_RECV_WAIT],
        event->step);
  }
}
...
__hidden void printProxyStepEventTrailer(FILE* fh, struct proxyStep* event) {
  if (event->isSend) {
    ...
    fprintf(
        ...
        getpid(),
        1,
        event->timestamp[PROXY_STEP_RECV_FLUSH_WAIT]);
    fprintf(
        fh,
        "{\"name\": \"%s\", \"cat\": \"NET\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"Step\": %d}},\n",
        "RecvFlushWait",
    ...
    fprintf(
        ...
        getpid(),
        1,
        event->timestamp[PROXY_STEP_RECV_GPU_WAIT]);
    fprintf(
        fh,
        "{\"name\": \"%s\", \"cat\": \"NET\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"Step\": %d}},\n",
        "RecvGpuWait",
...
__hidden void printKernelChEventHeader(FILE* fh, struct kernelCh* event) {
  if (event->type != ncclProfileKernelCh)
    return;
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"GPU\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"Channel\": %d, \"StartGpuClk\": %lu, \"StopGpuClk\": %lu}},\n",
      "KernelCh",
      kernelId,
      getpid(),
      1,
...
__hidden void printKernelChEventTrailer(FILE* fh, struct kernelCh* event) {
  ...
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"GPU\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
      "KernelCh",
      kernelId,
      getpid(),
      1,
...
__hidden void printProxyCtrlEvent(FILE* fh, struct proxyCtrl* event) {
  const char* str;
  if (event->state == ncclProfilerProxyCtrlIdle ||
      event->state == ncclProfilerProxyCtrlActive) {
    str = "Idle";
  } else if (
      event->state == ncclProfilerProxyCtrlSleep ||
      event->state == ncclProfilerProxyCtrlWakeup) {
    str = "Sleep";
  } else if (
      event->state == ncclProfilerProxyCtrlAppend ||
      event->state == ncclProfilerProxyCtrlAppendEnd) {
    str = "Append";
  } else {
    return;
  }
  if (event->state == ncclProfilerProxyCtrlAppendEnd) {
    fprintf(
        fh,
        "{\"name\": \"%s\", \"cat\": \"PROXY\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"appended\": %d}},\n",
        str,
        proxyCtrlId,
        getpid(),
        1,
    ...
    fprintf(
        fh,
        "{\"name\": \"%s\", \"cat\": \"PROXY\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
        str,
        proxyCtrlId,
        getpid(),
        1,
  ...
  if (event->pluginType == NCCL_PROFILER_NET_TYPE_IB) {
    if (event->pluginVer == 1) {
      if (event->pluginEvent == ncclProfileQp) {
        fprintf(
            fh,
            "{\"name\": \"%s\", \"cat\": \"NET_IB\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"device\": %d, \"qp_num\": %d, \"opcode\": %d, \"wr_id\": %lu, \"size\": %lu}},\n",
            "Qp",
            ibQpId,
            getpid(),
            1,
        ...
        fprintf(
            fh,
            "{\"name\": \"%s\", \"cat\": \"NET_IB\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
            "Qp",
            ibQpId++,
            getpid(),
            1,
    ...
    if (event->pluginVer == 1) {
      if (event->pluginEvent == ncclProfileSocket) {
        fprintf(
            fh,
            "{\"name\": \"%s\", \"cat\": \"NET_SOCK\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"sock\": %d, \"op\": %d, \"size\": %lu}},\n",
            "Sock",
            sockId,
            getpid(),
            1,
        ...
        fprintf(
            fh,
            "{\"name\": \"%s\", \"cat\": \"NET_SOCK\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
            "Sock",
            sockId++,
            getpid(),
            1,
...
static void
debugCeBatchEvent(FILE* fh, struct ceBatch* event, const char* tag) {
...
static void printCeCollEvent(FILE* fh, struct ceColl* event) {
  if (event->timingMode == CE_TIMING_GPU) {
    fprintf(
        fh,
        "{\"name\": \"%s\", \"cat\": \"CE_COLL\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"eventId\": %lu, \"count\": %lu, \"datatype\": \"%s\", \"strategy\": \"%s\", \"start_ts_cpu\": %f, \"stop_ts_cpu\": %f, \"duration_cpu_us\": %f, \"duration_gpu_us\": %lu}},\n",
        event->base.func,
        ceCollId,
        getpid(),
        1,
    ...
    fprintf(
        fh,
        "{\"name\": \"%s\", \"cat\": \"CE_COLL\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"eventId\": %lu, \"count\": %lu, \"datatype\": \"%s\", \"strategy\": \"%s\", \"start_ts_cpu\": %f, \"stop_ts_cpu\": %f, \"duration_cpu_us\": %f}},\n",
        event->base.func,
        ceCollId,
        getpid(),
        1,
  ...
  fprintf(
      fh,
      "{\"name\": \"%s\", \"cat\": \"CE_COLL\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
      event->base.func,
      ceCollId++,
      getpid(),
      1,
...
static void printCeSyncEvent(FILE* fh, struct ceSync* event) {
  ...
  if (event->timingMode == CE_TIMING_GPU) {
    fprintf(
        fh,
        "{\"name\": \"CeSync\", \"cat\": \"CE_SYNC\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"eventId\": %lu, \"type\": \"%s\", \"strategy\": \"%s\", \"nRanks\": %d, \"start_ts_cpu\": %f, \"stop_ts_cpu\": %f, \"duration_cpu_us\": %f, \"duration_gpu_us\": %lu}},\n",
        ceSyncId,
        getpid(),
        1,
        event->base.startTs,
    ...
    fprintf(
        fh,
        "{\"name\": \"CeSync\", \"cat\": \"CE_SYNC\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"eventId\": %lu, \"type\": \"%s\", \"strategy\": \"%s\", \"nRanks\": %d, \"start_ts_cpu\": %f, \"stop_ts_cpu\": %f, \"duration_cpu_us\": %f}},\n",
        ceSyncId,
        getpid(),
        1,
        event->base.startTs,
  ...
  fprintf(
      fh,
      "{\"name\": \"CeSync\", \"cat\": \"CE_SYNC\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
      ceSyncId++,
      getpid(),
      1,
      event->base.stopTs);
...
static void printCeBatchEvent(FILE* fh, struct ceBatch* event) {
  if (event->timingMode == CE_TIMING_GPU) {
    fprintf(
        fh,
        "{\"name\": \"CeBatch\", \"cat\": \"CE_BATCH\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"eventId\": %lu, \"numOps\": %d, \"totalBytes\": %lu, \"start_ts_cpu\": %f, \"stop_ts_cpu\": %f, \"duration_cpu_us\": %f, \"duration_gpu_us\": %lu}},\n",
        ceBatchId,
        getpid(),
        1,
        event->base.startTs,
    ...
    fprintf(
        fh,
        "{\"name\": \"CeBatch\", \"cat\": \"CE_BATCH\", \"ph\": \"b\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f, \"args\": {\"eventId\": %lu, \"numOps\": %d, \"totalBytes\": %lu, \"start_ts_cpu\": %f, \"stop_ts_cpu\": %f, \"duration_cpu_us\": %f}},\n",
        ceBatchId,
        getpid(),
        1,
        event->base.startTs,
  ...
  fprintf(
      fh,
      "{\"name\": \"CeBatch\", \"cat\": \"CE_BATCH\", \"ph\": \"e\", \"id\": %d, \"pid\": %d, \"tid\": %d, \"ts\": %f},\n",
      ceBatchId++,
      getpid(),
      1,
      event->base.stopTs);
...
void printEvent(FILE* fh, void* handle) {
  ...
  if (type == ncclProfileGroupApi) {
    ...
    struct kernelLaunch* kernelLaunchHead =
        profilerQueueHead(&g->kernelLaunchEvents);
