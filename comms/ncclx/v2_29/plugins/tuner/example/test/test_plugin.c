...
int test_config_parsing_valid() {
  ...
  TEST_ASSERT(
      result == ncclSuccess, "Plugin init with valid config should succeed");
...
int test_config_parsing_invalid() {
  ...
  TEST_ASSERT(
      result == ncclSuccess,
      "Plugin init should succeed even with invalid config");
...
int test_collective_matching() {
  ...
  ncclResult_t result = pluginGetCollInfo(
      context,
      ncclFuncAllReduce,
      32768,
      1,
  ...
  mock_logger(
      ...
      &cost_table[NCCL_ALGO_TREE][NCCL_PROTO_SIMPLE],
      cost_table[NCCL_ALGO_TREE][NCCL_PROTO_SIMPLE]);
  TEST_ASSERT(
      cost_table[NCCL_ALGO_TREE][NCCL_PROTO_SIMPLE] == 0.0,
      "Tree/Simple should have low cost");
  ...
  mock_logger(
      ...
      &cost_table[NCCL_ALGO_RING][NCCL_PROTO_LL128],
      cost_table[NCCL_ALGO_RING][NCCL_PROTO_LL128]);
  TEST_ASSERT(
      cost_table[NCCL_ALGO_RING][NCCL_PROTO_LL128] == 0.0,
      "Ring/LL128 should have low cost");
...
int test_size_matching() {
  ...
  TEST_ASSERT(
      cost_table[NCCL_ALGO_TREE][NCCL_PROTO_SIMPLE] == 0.0,
      "Small: Tree/Simple should have low cost");
  ...
  mock_logger(
      ...
      &cost_table[NCCL_ALGO_RING][NCCL_PROTO_SIMPLE],
      cost_table[NCCL_ALGO_RING][NCCL_PROTO_SIMPLE]);
  TEST_ASSERT(
      cost_table[NCCL_ALGO_RING][NCCL_PROTO_SIMPLE] == 0.0,
      "Medium: Ring/Simple should have low cost");
  ...
  TEST_ASSERT(
      cost_table[NCCL_ALGO_RING][NCCL_PROTO_LL128] == 0.0,
      "Large: Ring/LL128 should have low cost");
...
int test_topology_matching() {
  ...
  TEST_ASSERT(
      cost_table[NCCL_ALGO_TREE][NCCL_PROTO_SIMPLE] == 0.0,
      "Single node: Should match tree config");
  ...
  TEST_ASSERT(
      cost_table[NCCL_ALGO_RING][NCCL_PROTO_SIMPLE] == 0.0,
      "4-node: Should match ring/simple config");
...
int test_default_channels() {
  ...
  TEST_ASSERT(
      cost_table[NCCL_ALGO_TREE][NCCL_PROTO_SIMPLE] == 0.0,
      "Should apply algorithm/protocol");
  TEST_ASSERT(
      nChannels == 1, "Should keep default channels (1) when config has -1");
...
int test_regbuff_matching() {
  ...
  pluginGetCollInfo(
      ...
      NCCL_NUM_PROTOCOLS,
      1,
      &nChannels); // regBuff = 1 (registered)
  TEST_ASSERT(
      cost_table[NCCL_ALGO_TREE][NCCL_PROTO_SIMPLE] == 0.0,
      "Registered buffer: Tree/Simple should have low cost");
  ...
  pluginGetCollInfo(
      ...
      NCCL_NUM_PROTOCOLS,
      0,
      &nChannels); // regBuff = 0 (non-registered)
  TEST_ASSERT(
      cost_table[NCCL_ALGO_RING][NCCL_PROTO_SIMPLE] == 0.0,
      "Non-registered buffer: Ring/Simple should have low cost");
  TEST_ASSERT(nChannels == 4, "Non-registered buffer: Should set 4 channels");

  // Test backward compatibility - config without regBuff should match any
  // regBuff value
  for (int i = 0; i < NCCL_NUM_ALGORITHMS; i++) {
    for (int j = 0; j < NCCL_NUM_PROTOCOLS; j++) {
      cost_table[i][j] = 1.0;
  ...
  pluginGetCollInfo(
      ...
      NCCL_NUM_PROTOCOLS,
      2,
      &nChannels); // regBuff = 2 (only third config should match)
  TEST_ASSERT(
      cost_table[NCCL_ALGO_RING][NCCL_PROTO_LL128] == 0.0,
      "Any regBuff: Ring/LL128 should have low cost");
...
int test_pipeops_matching() {
  ...
  TEST_ASSERT(
      cost_table[NCCL_ALGO_TREE][NCCL_PROTO_SIMPLE] == 0.0,
      "Single pipeOp: Tree/Simple should have low cost");
  ...
  TEST_ASSERT(
      cost_table[NCCL_ALGO_RING][NCCL_PROTO_SIMPLE] == 0.0,
      "Multiple pipeOps: Ring/Simple should have low cost");
  TEST_ASSERT(nChannels == 4, "Multiple pipeOps: Should set 4 channels");

  // Test different number of pipeline ops (should match third config - backward
  // compatible)
  for (int i = 0; i < NCCL_NUM_ALGORITHMS; i++) {
    for (int j = 0; j < NCCL_NUM_PROTOCOLS; j++) {
      cost_table[i][j] = 1.0;
  ...
  TEST_ASSERT(
      cost_table[NCCL_ALGO_RING][NCCL_PROTO_LL128] == 0.0,
      "Any pipeOps: Ring/LL128 should have low cost");
...
int test_no_match_fallback() {
  ...
  mock_logger(
      ...
      cost_table[NCCL_ALGO_RING][NCCL_PROTO_SIMPLE]);
  TEST_ASSERT(
      cost_table[NCCL_ALGO_RING][NCCL_PROTO_SIMPLE] == 1.0,
      "Should use pass through unmodified");
...
int test_large_config() {
  ...
  fprintf(
      f,
      "# This file contains many configurations to test memory allocation\n");

  // Generate a large number of configurations (much more than the old
  // MAX_CONFIGS=100)
  const int num_configs = 500; // 5x the old static limit
  const char* collectives[] = {
  ...
  TEST_ASSERT(
      result == ncclSuccess, "Plugin init with large config should succeed");
  TEST_ASSERT(context != NULL, "Context should be allocated");

  // Verify that configurations were loaded
  TunerContext* ctx = (TunerContext*)context;
  TEST_ASSERT(
      ctx->numConfigs == num_configs,
      "Should load all configurations from large file");
  TEST_ASSERT(
      ctx->maxConfigs == num_configs, "maxConfigs should match allocated size");
  TEST_ASSERT(
      ctx->configs != NULL, "Configs array should be dynamically allocated");
  ...
  for (int i = 0; i < ctx->numConfigs; i++) {
    ...
    TEST_ASSERT(
        config->collType >= ncclFuncBroadcast &&
            config->collType <= ncclFuncAllReduce,
        "Collective type should be valid");
    TEST_ASSERT(
        config->maxBytes >= config->minBytes, "maxBytes should be >= minBytes");
  ...
  mock_logger(
      ...
      __FILE__,
      __LINE__,
      "Successfully loaded %d configurations (dynamic allocation)",
      ctx->numConfigs);
  mock_logger(
      NCCL_LOG_INFO,
      NCCL_ALL,
      ...
      ctx->maxConfigs,
      ctx->maxConfigs * sizeof(TuningConfig));

  // Test that the plugin can still find matching configurations from the large
  // set
  float cost_table[NCCL_NUM_ALGORITHMS][NCCL_NUM_PROTOCOLS];
  float* cost_table_ptr[NCCL_NUM_ALGORITHMS];
  for (int i = 0; i < NCCL_NUM_ALGORITHMS; i++) {
  ...
  result = pluginGetCollInfo(
      ...
      &nChannels);
  TEST_ASSERT(
      result == ncclSuccess, "GetCollInfo should work with large config set");
...
int test_very_large_config_stress() {
  ...
  for (int i = 0; i < stress_configs; i++) {
    ...
    fprintf(
        f,
        "allreduce,%d,%d,ring,simple,4,-1,-1,-1,-1\n",
        i * 512,
        (i * 512) + 1024);
  }
  ...
  ncclResult_t result = pluginInit(&context, 0, 8, 2, mock_logger, NULL, NULL);
  TEST_ASSERT(
      result == ncclSuccess, "Plugin should handle very large config files");

  TunerContext* ctx = (TunerContext*)context;
  TEST_ASSERT(
      ctx->numConfigs == stress_configs,
      "Should load all stress test configurations");
  ...
  mock_logger(
      ...
      __FILE__,
      __LINE__,
      "Stress test - loaded %d configurations successfully",
      stress_configs);
  mock_logger(
      NCCL_LOG_INFO,
      NCCL_ALL,
  ...
  for (int i = 0; i < stress_configs; i += 100) { // Sample every 100th config
    ...
    TEST_ASSERT(
        config->collType == ncclFuncAllReduce,
        "Config should have correct collective type");
    TEST_ASSERT(
        config->minBytes == (size_t)(i * 512),
        "Config should have correct minBytes");
  }
...
int test_empty_config() {
  ...
  TEST_ASSERT(
      ctx->configs == NULL, "Should not allocate memory for empty config");
  ...
  result = pluginGetCollInfo(
      ...
      &nChannels);
  TEST_ASSERT(
      result == ncclSuccess, "GetCollInfo should work with empty config");
...
int test_nvl_domain_info() {
  ...
  ncclNvlDomainInfo_v5_t nvl_domain = {
      ...
      .minRanksPerNvlDomain =
          3, // minimum ranks across all domains (bottleneck)
      .maxRanksPerNvlDomain = 5 // maximum ranks across all domains (capacity)
  };

  void* context = NULL;
  ncclResult_t result =
      pluginInit(&context, 0, 8, 2, mock_logger, &nvl_domain, NULL);
  TEST_ASSERT(
      result == ncclSuccess, "Plugin init with NVLink domains should succeed");

  // Validate NVLD info structure
  TEST_ASSERT(nvl_domain.nNvlDomains == 2, "Should have 2 domains (nodes)");
  TEST_ASSERT(
      nvl_domain.minRanksPerNvlDomain == 3,
      "Should have minimum 3 ranks per domain");
  TEST_ASSERT(
      nvl_domain.maxRanksPerNvlDomain == 5,
      "Should have maximum 5 ranks per domain");
...
int test_tuner_constants() {
  ...
  ncclTunerConstants_v5_t constants = {
      ...
      // Hardware latencies:
      // [NCCL_NUM_HW_LINKS][NCCL_NUM_ALGORITHMS][NCCL_NUM_PROTOCOLS]
      .hwLatencies =
          {// NCCL_HW_NVLINK
           {
               {-1.0, -1.0, -1.0}, // TREE
      ...
      // Per-channel maximum Ring LL128 bandwidths:
      ...
      // Per-channel maximum Tree LL128 bandwidths:
      // [NCCL_NUM_COMPCAPS][NCCL_NUM_TUNING_SCALES]
      ...
      // Per-channel maximum Tree bandwidths:
      // [NCCL_NUM_COMPCAPS][NCCL_NUM_TUNING_SCALES]
      .perChMaxTreeBws = {
          {-1.0, -1.0, -1.0}, // Volta: 1node, 2nodes, 4nodes
          {-1.0, -1.0, -1.0}, // Ampere: 1node, 2nodes, 4nodes
  ...
  void* context = NULL;
  ncclResult_t result =
      pluginInit(&context, 0, 8, 2, mock_logger, NULL, &constants);
  TEST_ASSERT(
      result == ncclSuccess, "Plugin init with constants should succeed");
...
TestCase test_cases[] = {
    ...
    {"large-config",
     test_large_config,
     "Large configuration files (dynamic allocation)"},
    {"stress-config",
     test_very_large_config_stress,
     "Very large configuration stress test"},
...
void show_help(const char* program_name) {
  ...
  printf(
      "  %s init               # Run only initialization test\n", program_name);
  printf(
      "  %s init collective    # Run initialization and collective tests\n",
      program_name);
